...
const CursorTrail = () => {
  ...
  const throttleDelay = 50; // Update every 50ms (20fps) instead of every frame
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      // Update trail - limit array size immediately to prevent growth
      const newPoint = { x, y, id: `${now}` };
      trailRef.current = [newPoint, ...trailRef.current].slice(0, trailLength);
      ...
      <AnimatePresence>
        {isVisible && (
          <>
            ...
            {/* Pulse Effect on Hover (removed infinite repeat to prevent memory leak) */}
            {isHovering && (
              <motion.div
